...
class OverallController extends Controller
{
     ...
     * Grand Total = Sum of all weighted category results (built-in + custom dynamic).
     */
    public function index(Request $request)
    {
        ...
        foreach ($candidates as $c) {
            ...
            // Add custom category weighted scores
            $customBreakdown = [];
            foreach ($customCategories as $customCat) {
            ...
            foreach ($judges as $j) {
                ...
                foreach ($customCategories as $customCat) {
                    ...
                    $customJudgeScores[$customCat->key] = isset($raw[$key]) ? (float) $raw[$key]->score : null;
                }
        ...
        foreach ($customCategories as $customCat) {
            $raw = $customRawScores[$customCat->key] ?? collect();
            foreach ($raw as $k => $s) {
                $rawScoresMap['custom:'.$customCat->key.'_'.$k] = (float) $s->score;
            }
        }
        ...
        return view('admin.overall.index', compact(
            'candidates',
            'breakdown',
            'judges',
            'judgeBreakdown',
