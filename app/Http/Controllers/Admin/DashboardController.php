...
class DashboardController extends Controller
{
    ...
    private function getDashboardData(): array
    {
        ...
        foreach ($candidateRankings as $cr) {
            if ($cr['total_score'] > 0) {
                if (!$leadingCandidate) $leadingCandidate = $cr;
                if (!$leadingMale && $cr['gender'] === 'Male') $leadingMale = $cr;
                if (!$leadingFemale && $cr['gender'] === 'Female') $leadingFemale = $cr;
            }
        }
        ...
        if ($expectedPerJudge === 0) $expectedPerJudge = 1;
