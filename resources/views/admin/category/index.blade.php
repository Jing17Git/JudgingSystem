...
@section('title', $categorySetting->name . ' — Scoring Table')
...
<style>
    @media print {
        ...
        .gender-section { display: none !important; }
        .gender-section.print-visible { display: block !important; }
        ...
        .data-table th, .data-table td { border: 1px solid #000 !important; padding: 6px 8px !important; font-size: 10pt !important; color: #000 !important; }
        .data-table th { background-color: #f3f4f6 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
        .print-signatures { display: grid !important; grid-template-columns: repeat(auto-fit, minmax(130px, 1fr)) !important; gap: 20px 24px; margin-top: 40px; page-break-inside: avoid; }
    }
    .print-header, .print-signatures { display: none; }

    @keyframes cellScoreFlash {
        0% { background-color: #fde047; transform: scale(1.18); box-shadow: 0 0 14px rgba(234,179,8,.5); }
        45% { background-color: #86efac; transform: scale(1.06); }
        100% { background-color: transparent; transform: scale(1); box-shadow: none; }
    }
    .cell-flash { animation: cellScoreFlash 1.6s ease-out; }

    @keyframes cellResetFlash {
        0% { background-color: #fecdd3; transform: scale(1.15); box-shadow: 0 0 14px rgba(244,63,94,.4); }
        45% { background-color: #fef08a; transform: scale(1.05); }
        100% { background-color: transparent; transform: scale(1); box-shadow: none; }
    }
    .cell-reset-flash { animation: cellResetFlash 1.6s ease-out; }
    ...
    .rank-pop { animation: rankBadgePop 0.85s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
</style>
@endpush
...
<div class="page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="page-title flex items-center gap-3">
            ...
            {{ $categorySetting->name }} — Scoring Table
        </h1>
    ...
    <div class="flex items-center gap-3">
        ...
        <span class="text-xs bg-green-100 text-green-800 border border-green-200 px-3 py-1.5 rounded-lg font-semibold">
            {{ (int) $categorySetting->percentage }}% weight
        </span>
    </div>
</div>
...
<div class="print-header">
    ...
    <p style="font-size: 13pt; margin: 4px 0 0; font-weight: bold;">{{ $categorySetting->name }} — Scoring Table</p>
...
@foreach($groups as $gKey => $group)
    ...
    <div class="mb-8 animate-fade-in-up gender-section" id="section-{{ strtolower($gKey) }}">
        <div class="flex items-center gap-3 mb-3">
            ...
            @endif
            @if($gKey !== 'Unset')
                <button onclick="printGenderSection('{{ strtolower($gKey) }}', '{{ $categorySetting->name }}', '{{ $group['label'] }}')"
                    ...
                    @foreach($sortedGroup as $candidate)
                        ...
                        <tr id="row-cand-{{ $candidate->id }}"
                            ...
                            <td class="sticky left-0 bg-white py-3 px-4">
                                @if($gKey === 'Male')
                                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-full text-white text-base font-black shadow-md" style="background-color: #2563eb; border: 2px solid #60a5fa;">{{ $candidate->candidate_number }}</span>
                                @elseif($gKey === 'Female')
                                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-full text-white text-base font-black shadow-md" style="background-color: #db2777; border: 2px solid #f472b6;">{{ $candidate->candidate_number }}</span>
                                @else
                                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-full text-white text-base font-black shadow-md" style="background-color: #4b5563; border: 2px solid #9ca3af;">{{ $candidate->candidate_number }}</span>
                                @endif
                            </td>
                            <td class="sticky left-[110px] bg-white py-3 px-4 font-semibold text-gray-900">
                                <div class="flex items-center gap-3">
                                    ...
                                    @else
                                        @if($gKey === 'Male')
                                            <div class="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 shadow-sm" style="background-color: #2563eb;">{{ strtoupper(substr($candidate->display_name, 0, 1)) }}</div>
                                        @elseif($gKey === 'Female')
                                            <div class="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 shadow-sm" style="background-color: #db2777;">{{ strtoupper(substr($candidate->display_name, 0, 1)) }}</div>
                                        @else
                                            <div class="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 shadow-sm" style="background-color: #4b5563;">{{ strtoupper(substr($candidate->display_name, 0, 1)) }}</div>
                                        @endif
                                    @endif
                            ...
                            @foreach($judges as $judge)
                                @php
                                    $scoreKey      = $candidate->id . '_' . $judge->id;
                                    $existingScore = isset($scores[$scoreKey]) ? $scores[$scoreKey]->score : null;
                                @endphp
                            ...
                            <td id="rank-cell-{{ $candidate->id }}" class="text-center py-3 px-4" style="background-color: #fffbeb;">
                                <div id="rank-val-{{ $candidate->id }}" class="inline-flex items-center justify-center transition-all">
                                    ...
                                    @elseif($rank === 1)
                                        <span class="inline-flex items-center justify-center w-11 h-11 rounded-full text-2xl shadow-lg ring-4 ring-amber-300/70 bg-gradient-to-tr from-amber-500 via-yellow-400 to-amber-300 text-white" title="1st Place">🥇</span>
                                    @elseif($rank === 2)
                                    ...
                                    @elseif($rank === 4)
                                        <span class="inline-flex items-center justify-center w-9 h-9 rounded-full text-sm font-black shadow-md ring-2 ring-blue-300 bg-gradient-to-tr from-blue-600 to-indigo-600 text-white">4</span>
                                    @elseif($rank === 5)
                                        <span class="inline-flex items-center justify-center w-9 h-9 rounded-full text-sm font-black shadow-md ring-2 ring-purple-300 bg-gradient-to-tr from-purple-600 to-pink-600 text-white">5</span>
                                    @else
...
<script>
    ...
    const CATEGORY_NAME = '{{ $categorySetting->name }}';
    const POLL_URL = '{{ route("admin.category.index", ["key" => $key]) }}';

    function printGenderSection(gender, category, label) {
        document.querySelectorAll('.gender-section').forEach(el => el.classList.remove('print-visible'));
        const target = document.getElementById('section-' + gender);
        if (target) target.classList.add('print-visible');
        const printHeader = document.querySelector('.print-header');
        if (printHeader) printHeader.querySelector('p:nth-child(2)').textContent = category + ' — ' + label;
        window.print();
        const cleanup = () => {
            document.querySelectorAll('.gender-section').forEach(el => el.classList.remove('print-visible'));
    ...
    async function pollScoresFallback() {
        try {
            const res = await fetch(POLL_URL, { headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'application/json' } });
            if (!res.ok) return;
            ...
            if (data.success && data.scores) {
                for (const key in data.scores) {
                    ...
                    if (parts.length === 2) {
                        ...
                        const newScore = parseFloat(data.scores[key]);
                        if (SCORES_STORE[key] === undefined || SCORES_STORE[key] !== newScore) {
                            const cand  = CANDIDATES_DATA.find(c => c.id === candId);
                            const judge = JUDGES_DATA.find(j => j.id === judgeId);
                            handleLiveScoreChange(candId, judgeId, newScore,
                                judge ? judge.name : 'Judge', cand ? cand.display_name : 'Candidate',
                                cand ? cand.candidate_number : candId, 'saved', false);
                        }
                    }
                }
                for (const key in SCORES_STORE) {
                    if (!(key in data.scores)) {
                        ...
                        const cand  = CANDIDATES_DATA.find(c => c.id === candId);
                        const judge = JUDGES_DATA.find(j => j.id === judgeId);
                        handleLiveScoreChange(candId, judgeId, null,
                            judge ? judge.name : 'Judge', cand ? cand.display_name : 'Candidate',
                            cand ? cand.candidate_number : candId, 'reset', false);
                    }
                }
            }
        } catch (e) { /* silent */ }
    ...
    function handleLiveScoreChange(candId, judgeId, newScore, judgeName, candName, candNum, action = 'saved', triggerToast = true) {
        ...
        const isReset = (action === 'reset' || newScore === null || isNaN(newScore));
        if (!isReset) { SCORES_STORE[key] = parseFloat(newScore); } else { delete SCORES_STORE[key]; }
        ...
        if (scoreValEl) {
            if (!isReset) {
                ...
                setTimeout(() => { scoreValEl.className = 'inline-flex items-center justify-center w-10 h-7 rounded-md bg-gray-100 text-sm font-bold text-gray-800 transition-all'; }, 1800);
            } else {
                ...
                setTimeout(() => { scoreValEl.className = 'text-gray-300 font-medium transition-all'; }, 1800);
            }
        }

        let candSum = 0;
        JUDGES_DATA.forEach(j => { const s = SCORES_STORE[`${candId}_${j.id}`]; if (s !== undefined && !isNaN(s)) candSum += parseFloat(s); });
        ...
        if (candidateObj) recalculateGenderRanks(candidateObj.gender);

        if (triggerToast && window.showToast) {
            const formattedCandNum = String(candNum || '').padStart(2, '0');
            if (isReset) {
                window.showToast('Score Reset', `${judgeName} reset score for Candidate #${formattedCandNum} (${candName})`, 'warning', 4000);
            } else {
                window.showToast(`${CATEGORY_NAME} Score Recorded`, `${judgeName} scored ${parseFloat(newScore).toFixed(1)} for Candidate #${formattedCandNum} (${candName})`, 'success', 4000);
            }
        }
    }
    ...
    function recalculateGenderRanks(gender) {
        ...
        const totals = groupCandidates.map(c => {
            let sum = 0;
            JUDGES_DATA.forEach(j => { const s = SCORES_STORE[`${c.id}_${j.id}`]; if (s !== undefined) sum += parseFloat(s); });
            return { id: c.id, candidate_number: c.candidate_number, total: JUDGE_COUNT > 0 ? sum / JUDGE_COUNT : 0 };
        });
        totals.sort((a, b) => {
            if (a.total > 0 && b.total > 0) { if (Math.abs(a.total - b.total) < 0.0001) return a.candidate_number - b.candidate_number; return b.total - a.total; }
        ...
        totals.forEach(item => {
            ...
            let rankHtml = '';
            if (!rank) rankHtml = `<span class="text-gray-400 font-medium">—</span>`;
            else if (rank === 1) rankHtml = `<span class="inline-flex items-center justify-center w-11 h-11 rounded-full text-2xl shadow-lg ring-4 ring-amber-300/70 bg-gradient-to-tr from-amber-500 via-yellow-400 to-amber-300 text-white rank-pop">🥇</span>`;
            else if (rank === 2) rankHtml = `<span class="inline-flex items-center justify-center w-10 h-10 rounded-full text-xl shadow-md ring-2 ring-slate-300 bg-gradient-to-tr from-slate-400 via-gray-300 to-slate-200 text-slate-800 rank-pop">🥈</span>`;
            else if (rank === 3) rankHtml = `<span class="inline-flex items-center justify-center w-10 h-10 rounded-full text-xl shadow-md ring-2 ring-orange-300 bg-gradient-to-tr from-amber-700 via-orange-500 to-amber-400 text-white rank-pop">🥉</span>`;
            else if (rank === 4) rankHtml = `<span class="inline-flex items-center justify-center w-9 h-9 rounded-full text-sm font-black shadow-md ring-2 ring-blue-300 bg-gradient-to-tr from-blue-600 to-indigo-600 text-white rank-pop">4</span>`;
            else if (rank === 5) rankHtml = `<span class="inline-flex items-center justify-center w-9 h-9 rounded-full text-sm font-black shadow-md ring-2 ring-purple-300 bg-gradient-to-tr from-purple-600 to-pink-600 text-white rank-pop">5</span>`;
            else rankHtml = `<span class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 border-2 border-gray-300 text-gray-700 text-xs font-extrabold shadow-sm rank-pop">${rank}</span>`;
            if (rankValEl.innerHTML.trim() !== rankHtml.trim()) rankValEl.innerHTML = rankHtml;
        });
        const tbody = document.getElementById(`tbody-${gender.toLowerCase()}`);
        if (tbody) totals.forEach(item => { const row = document.getElementById(`row-cand-${item.id}`); if (row) tbody.appendChild(row); });
    }
</script>
@endpush
@endsection
