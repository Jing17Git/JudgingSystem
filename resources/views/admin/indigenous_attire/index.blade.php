...
<div class="page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="page-title flex items-center gap-3">
            ...
            Indigenous Attire — Scoring Table
        </h1>
...
<div class="print-header">
    ...
    <p style="font-size: 13pt; margin: 4px 0 0; font-weight: bold;">Indigenous Attire — Scoring Table</p>
...
<div id="indigenous-attire-table-container">
@php
    $groups = [
...
@foreach($groups as $gKey => $group)
    ...
    <div class="mb-8 animate-fade-in-up gender-section" id="section-{{ strtolower($gKey) }}">
        ...
        <div class="flex items-center gap-3 mb-3">
            ...
            {{-- Print Button --}}
            @if($gKey !== 'Unset')
                <button onclick="printGenderSection('{{ strtolower($gKey) }}', 'Indigenous Attire', '{{ $group['label'] }}')"
...
<script>
    ...
    function setupIndigenousEcho() {
        if (!window.Echo) {
            updateStatusIndicator('connecting');
            setTimeout(setupIndigenousEcho, 400);
            return;
        }
        ...
        try {
            window.Echo.private('admin.scores')
                .subscribed(() => {
                    ...
                    console.log('⚡ Connected to admin.scores channel on Indigenous Attire table');
                })
                .error((err) => {
                ...
                .listen('.score.submitted', (e) => {
                    if (e.category === 'indigenous-attire' || e.category === 'indigenous_attire') {
    ...
    async function pollScoresFallback() {
        try {
            const res = await fetch('{{ route("admin.indigenous-attire.index") }}', {
                headers: {
    ...
    function handleLiveScoreChange(candId, judgeId, newScore, judgeName, candName, candNum, action = 'saved', triggerToast = true) {
        ...
        if (triggerToast && window.showToast) {
            ...
            if (isReset) {
                ...
                window.showToast(
                    'Indigenous Attire Score Recorded',
