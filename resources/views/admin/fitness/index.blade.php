...
<div id="fitness-table-container">
@php
    $groups = [
...
@push('scripts')
<script>
    // Print gender section function (preserves print functionality)
    ...
    function setupFitnessEcho() {
        if (!window.Echo) {
            updateStatusIndicator('connecting');
            setTimeout(setupFitnessEcho, 400);
            return;
        }
        ...
        try {
            window.Echo.private('admin.scores')
                .subscribed(() => {
                    ...
                    console.log('⚡ Connected to admin.scores channel on Fitness table');
                })
                .error((err) => {
    ...
    // Fallback polling for maximum reliability when socket is connecting or offline
    ...
    async function pollScoresFallback() {
        try {
            const res = await fetch('{{ route("admin.fitness.index") }}', {
                headers: {
    ...
    /**
     * Update only the affected cells (Score, Total, Rank) with smooth flash animations
     */
    function handleLiveScoreChange(candId, judgeId, newScore, judgeName, candName, candNum, action = 'saved', triggerToast = true) {
        ...
        // 2. Update Specific Judge Score Cell
        ...
        if (totalValEl) {
            ...
            void totalValEl.offsetWidth; // trigger reflow
        ...
        // 4. Recalculate Ranks for the entire Gender Group
    ...
    /**
     * Compute competition rank (1, 2, 2, 4...) for a gender group and re-sort table rows
     */
    function recalculateGenderRanks(gender) {
        ...
        // Calculate current total for all candidates in group
        const totals = [];
        ...
        // Sort: total descending, tie-breaker by candidate_number ascending
        totals.sort((a, b) => {
            if (a.total > 0 && b.total > 0) {
                if (a.total === b.total) return a.candidate_number - b.candidate_number;
        ...
        // Assign dense ranks (no skipping on ties)
        let r = 0;
        let prev = null;
        const ranks = {};
        ...
        // Re-order tbody table rows according to new rankings
