...
<div class="panel animate-fade-in-up delay-100 max-w-2xl">
    <div class="panel-body">
        <form action="{{ route('admin.candidates.update', $candidate) }}" method="POST" enctype="multipart/form-data" id="candidateForm">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="full_name" name="full_name" class="form-input @error('full_name') border-[var(--danger)] @enderror" value="{{ old('full_name', $candidate->display_name) }}" required minlength="3" maxlength="100" style="text-transform: capitalize;">
                    @error('full_name')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="gender" name="gender" class="form-input @error('gender') border-[var(--danger)] @enderror" required>
                        <option value="" disabled {{ old('gender', $candidate->gender) ? '' : 'selected' }}>Select gender...</option>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="origin" name="origin" class="form-input @error('origin') border-[var(--danger)] @enderror" value="{{ old('origin', $candidate->origin) }}" placeholder="e.g. BSIT, BSAB, BEED">
                    @error('origin')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 border border-[var(--border-default)] rounded-2xl bg-gray-50/50">
                            ...
                            <div class="relative group">
                                <div class="w-20 h-20 rounded-2xl border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden bg-white flex-shrink-0 shadow-sm transition-all" id="previewContainer">
                                    @if($candidate->photo_url)
                                        <img id="previewImage" src="{{ asset('storage/' . $candidate->photo_url) }}" class="w-full h-full object-cover rounded-xl" alt="{{ $candidate->display_name }}">
                                        <div id="placeholderIcon" class="hidden flex flex-col items-center justify-center text-gray-400">
                                    ...
                                    @else
                                        ...
                                        <div id="placeholderIcon" class="w-full h-full bg-gradient-to-br from-green-500 to-emerald-700 flex items-center justify-center text-white text-xl font-bold rounded-xl">
                            ...
                            <div class="flex-1 min-w-0 w-full space-y-2">
                                ...
                                <div class="flex items-center justify-between gap-2 flex-wrap">
                                    <p class="text-xs text-[var(--text-muted)]">Leave empty to keep current photo. Max 2MB.</p>
                                    <button type="button" id="openCropperBtn" class="{{ $candidate->photo_url ? '' : 'hidden' }} btn btn-outline btn-sm text-xs font-semibold py-1.5 px-3 rounded-lg border-green-400 text-green-700 hover:bg-green-50 flex items-center gap-1.5">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let cropper = null;
    let originalImageSrc = '{{ $candidate->photo_url ? asset('storage/' . $candidate->photo_url) : '' }}';
