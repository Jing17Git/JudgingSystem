        ...
        <form action="{{ route('admin.candidates.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="space-y-6">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="full_name" name="full_name" class="form-input @error('full_name') border-[var(--danger)] @enderror" value="{{ old('full_name') }}" required autofocus placeholder="e.g. Maria Santos" minlength="3" maxlength="100">
                    @error('full_name')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div x-data="{ preview: null }" class="space-y-3">
                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                            ...
                            <div class="flex-1 min-w-0 w-full">
                                ...
                                <p class="text-xs text-[var(--text-muted)] mt-1.5">Accepted formats: JPEG, PNG, WebP. Max size: 2MB.</p>
                            </div>
                        </div>
                    </div>
                    @error('picture')
