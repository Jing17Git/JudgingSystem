...
@section('title', 'Production Scoring')
...
<div class="print-header">
    ...
    <p style="font-size: 13pt; margin: 4px 0 0; font-weight: bold;">Production — Scoring Table</p>
...
@foreach($groups as $gKey => $group)
    ...
    <div class="mb-8 animate-fade-in-up gender-section" id="section-{{ strtolower($gKey) }}">
        ...
        <div class="flex items-center gap-3 mb-3">
            ...
            {{-- Print Button --}}
            @if($gKey !== 'Unset')
                <button onclick="printGenderSection('{{ strtolower($gKey) }}', 'Production', '{{ $group['label'] }}')"
...
<script>
    ...
    function setupProductionEcho() {
        if (!window.Echo) {
            updateStatusIndicator('connecting');
            setTimeout(setupProductionEcho, 400);
            return;
        }
        ...
        try {
            window.Echo.private('admin.scores')
                .subscribed(() => {
                    ...
                    console.log('⚡ Connected to admin.scores channel on Production table');
                })
                .error((err) => {
    ...
    async function pollScoresFallback() {
        try {
            const res = await fetch('{{ route("admin.production.index") }}', {
                headers: {
