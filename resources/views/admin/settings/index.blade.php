...
@section('title', 'Criteria Settings')
...
<div class="page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        <div class="flex items-center gap-2 text-xs text-[var(--text-muted)] mb-1">
            <span>Settings</span>
            <span>/</span>
            <span class="text-[var(--green-700)] font-semibold">Preliminary</span>
        </div>
        ...
        <p class="page-subtitle">Configure percentage weight for Preliminary judging categories (Production, Fitness, Indigenous Attire, Traditional Attire). Overall score calculates each category's average score multiplied by its percentage.</p>
    </div>
</div>
...
<div class="max-w-4xl" x-data="{
    ...
    <div class="mb-6 p-4 rounded-xl border border-blue-100 bg-blue-50/70 text-blue-900 text-sm flex items-start gap-3 shadow-sm">
        ...
        <div>
            <span class="font-bold">Settings Rule:</span>
            Each criteria must be a <strong>whole number percentage</strong> (no decimals) and the total sum of all 4 criteria must equal <strong>exactly 100%</strong>.
            <div class="mt-1 text-xs text-blue-800 font-mono">
                Category Result = Judges Average × (Percentage ÷ 100)
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.settings.update') }}" method="POST">
        ...
        <div class="panel border border-[var(--border-default)] shadow-sm rounded-xl overflow-hidden mb-6">
            <div class="panel-header px-6 py-4 border-b border-[var(--border-default)] bg-[var(--bg-card)] flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 class="text-base font-bold text-[var(--text-primary)]">Category Weights (Whole Numbers Only)</h2>
                    <p class="text-xs text-[var(--text-muted)]">Set the percentage weight of each category. Must total 100%.</p>
                </div>
            ...
            <div class="panel-footer px-6 py-4 border-t border-[var(--border-default)] bg-[var(--bg-card)] flex flex-wrap justify-between items-center gap-3">
                <a href="{{ route('admin.overall.index') }}" class="btn btn-outline">View Overall Results</a>
