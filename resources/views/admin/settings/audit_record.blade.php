...
<div x-data="{
    ...
    {{-- Page Header --}}
    <div class="page-header flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-5 sm:mb-6">
        <div>
            ...
            <h1 class="page-title flex items-center gap-2.5 sm:gap-3 text-lg sm:text-xl md:text-2xl font-bold text-gray-900">
                <svg class="w-6 h-6 sm:w-7 sm:h-7 text-emerald-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/>
                </svg>
                Audit Management &amp; Security Threat Engine
            </h1>
            <p class="page-subtitle text-xs sm:text-sm mt-1">Track judge score recordings, score resets, automated anomaly alerts, and threat indicators in real-time.</p>
        </div>
        <div class="flex flex-wrap items-center gap-2 sm:gap-2.5 w-full lg:w-auto">
            ...
            <button @click="toggleAutoRefresh()"
                    ...
                    class="btn text-xs font-semibold px-3 py-2 sm:px-3.5 sm:py-2 rounded-xl border flex items-center justify-center gap-2 transition-all flex-1 sm:flex-initial">
                <span class="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full flex-shrink-0" :class="autoRefresh ? 'bg-white animate-pulse' : 'bg-gray-400'"></span>
                <span x-text="autoRefresh ? 'Live Monitoring ON' : 'Live Auto-Refresh'"></span>
            </button>

            {{-- Export CSV --}}
            <a href="{{ route('admin.settings.audit_record.export') }}" class="btn btn-outline flex items-center justify-center gap-1.5 sm:gap-2 text-xs font-semibold shadow-sm bg-white hover:bg-emerald-50 px-3 py-2 sm:px-3.5 sm:py-2 flex-1 sm:flex-initial">
            ...
            {{-- Clear Audit History --}}
            <form action="{{ route('admin.settings.audit_record.clear') }}" method="POST" onsubmit="return confirm('Are you sure you want to clear all audit records? This action cannot be undone.');" class="inline flex-1 sm:flex-initial">
                @csrf
                <button type="submit" class="w-full btn btn-outline text-rose-600 hover:bg-rose-50 border-rose-200 text-xs font-semibold flex items-center justify-center gap-1.5 shadow-sm bg-white px-3 py-2 sm:px-3.5 sm:py-2">
    ...
    <div class="flex flex-nowrap overflow-x-auto items-center gap-1.5 sm:gap-2 mb-5 sm:mb-6 border-b border-[var(--border-default)] pb-2.5 sm:pb-3 hide-scrollbar -webkit-overflow-scrolling-touch">
        <a href="{{ route('admin.settings.preliminary') }}"
           class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-semibold text-[var(--text-secondary)] hover:bg-gray-100 transition-colors flex items-center gap-2 whitespace-nowrap">
            <span class="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-gray-400"></span>
        ...
        <a href="{{ route('admin.settings.audit_record') }}"
           class="px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-bold bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center gap-2 shadow-sm whitespace-nowrap">
            <span class="w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full bg-emerald-600"></span>
            Audit Record
        </a>
    ...
    {{-- Flash Messages --}}
    @if(session('success'))
        <div class="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl px-4 py-3 mb-5 sm:mb-6 text-xs sm:text-sm flex items-center gap-2 shadow-sm">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-4 mb-5 sm:mb-6">
        <div class="panel p-3 sm:p-4 border border-gray-200 shadow-sm rounded-xl bg-white">
            <p class="text-[10px] sm:text-[11px] font-bold text-gray-500 uppercase tracking-wider truncate">Total Logs</p>
            <p class="text-xl sm:text-2xl font-black text-gray-900 mt-0.5 sm:mt-1">{{ number_format($stats['total_records']) }}</p>
        </div>
        <div class="panel p-3 sm:p-4 border border-gray-200 shadow-sm rounded-xl bg-white">
            <p class="text-[10px] sm:text-[11px] font-bold text-emerald-600 uppercase tracking-wider truncate">Submissions</p>
            <p class="text-xl sm:text-2xl font-black text-emerald-600 mt-0.5 sm:mt-1">{{ number_format($stats['score_submissions']) }}</p>
        </div>
        <div class="panel p-3 sm:p-4 border border-gray-200 shadow-sm rounded-xl bg-white">
            <p class="text-[10px] sm:text-[11px] font-bold text-rose-600 uppercase tracking-wider truncate">Score Resets</p>
            <p class="text-xl sm:text-2xl font-black text-rose-600 mt-0.5 sm:mt-1">{{ number_format($stats['score_resets']) }}</p>
        </div>
        <div class="panel p-3 sm:p-4 border border-amber-200 bg-amber-50/50 shadow-sm rounded-xl">
            <p class="text-[10px] sm:text-[11px] font-bold text-amber-700 uppercase tracking-wider truncate">Suspicious</p>
            <p class="text-xl sm:text-2xl font-black text-amber-700 mt-0.5 sm:mt-1">{{ number_format($stats['flagged_suspicious']) }}</p>
        </div>
        <div class="panel p-3 sm:p-4 border border-rose-200 bg-rose-50/50 shadow-sm rounded-xl">
            <p class="text-[10px] sm:text-[11px] font-bold text-rose-700 uppercase tracking-wider truncate">Threats</p>
            <p class="text-xl sm:text-2xl font-black text-rose-700 mt-0.5 sm:mt-1">{{ number_format($stats['critical_alerts']) }}</p>
        </div>
        <div class="panel p-3 sm:p-4 border border-gray-200 shadow-sm rounded-xl bg-white">
            <p class="text-[10px] sm:text-[11px] font-bold text-indigo-600 uppercase tracking-wider truncate">Judges Audited</p>
            <p class="text-xl sm:text-2xl font-black text-indigo-600 mt-0.5 sm:mt-1">{{ number_format($stats['unique_judges']) }}</p>
        </div>
    </div>

    {{-- Filter & Search Panel --}}
    <div class="panel p-3.5 sm:p-4 mb-5 sm:mb-6 border border-gray-200 shadow-sm rounded-xl bg-white">
        <form action="{{ route('admin.settings.audit_record') }}" method="GET" class="flex flex-col gap-3">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-2.5 sm:gap-3">
                ...
                <div class="sm:col-span-2 lg:col-span-2">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="Search judge, candidate, IP, reason..."
                           class="w-full px-3 sm:px-3.5 py-2 text-xs sm:text-sm border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none">
                </div>

                {{-- Category --}}
                <div>
                    <label class="block text-[11px] font-bold text-gray-500 uppercase mb-1">Category</label>
                    <select name="category" class="w-full text-xs sm:text-sm px-3 sm:px-3.5 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                ...
                <div>
                    ...
                    <select name="risk_level" class="w-full text-xs sm:text-sm px-3 sm:px-3.5 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
                ...
                <div>
                    ...
                    <select name="date_preset" class="w-full text-xs sm:text-sm px-3 sm:px-3.5 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none bg-white">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2.5 border-t border-gray-100">
                ...
                <label class="inline-flex items-center gap-2 cursor-pointer text-xs font-bold text-gray-700">
                    ...
                    <span class="text-amber-800 font-extrabold flex items-center gap-1 text-xs">
    ...
    <div class="overflow-x-auto border border-gray-200 shadow-sm rounded-xl bg-white mb-6 -webkit-overflow-scrolling-touch">
        ...
        @else
            <table class="min-w-full border-collapse text-xs sm:text-sm">
                <thead>
                    <tr class="bg-emerald-800 text-white border-b-2 border-emerald-900 whitespace-nowrap">
                        <th class="py-3 px-3 sm:px-4 text-left text-xs font-bold uppercase tracking-wider min-w-[140px] sm:w-[160px] border-r border-emerald-700/50">
                            Timestamp
                        </th>
                        <th class="py-3 px-3 sm:px-4 text-left text-xs font-bold uppercase tracking-wider min-w-[130px] sm:w-[160px] border-r border-emerald-700/50">
                            User Details
                        </th>
                        <th class="py-3 px-3 sm:px-4 text-left text-xs font-bold uppercase tracking-wider min-w-[130px] sm:w-[160px] border-r border-emerald-700/50">
                            Candidate Details
                        </th>
                        <th class="py-3 px-3 sm:px-4 text-left text-xs font-bold uppercase tracking-wider min-w-[140px] sm:w-[170px] border-r border-emerald-700/50">
                            Category &amp; Score
                        </th>
                        <th class="py-3 px-3 sm:px-4 text-left text-xs font-bold uppercase tracking-wider min-w-[200px] sm:min-w-[240px] border-r border-emerald-700/50">
                            Action Taken / Description
                        </th>
                        <th class="py-3 px-3 sm:px-4 text-left text-xs font-bold uppercase tracking-wider min-w-[125px] sm:w-[145px] border-r border-emerald-700/50">
                            Device &amp; IP
                        </th>
                        <th class="py-3 px-3 sm:px-4 text-left text-xs font-bold uppercase tracking-wider min-w-[120px] sm:w-[140px] border-r border-emerald-700/50">
                            Threat Indicator
                        </th>
                        <th class="py-3 px-3 sm:px-4 text-center text-xs font-bold uppercase tracking-wider min-w-[90px] sm:w-[105px]">
                            Actions
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @foreach($auditRecords as $i => $record)
                        ...
                        <tr class="hover:bg-emerald-50/40 transition-colors {{ $record->is_suspicious ? 'bg-amber-50/40' : ($isEven ? 'bg-white' : 'bg-gray-50/50') }}">
                            ...
                            {{-- Col 1: Timestamp --}}
                            <td class="py-3 px-3 sm:px-4 align-top border-r border-gray-100">
                                <div class="flex flex-col gap-0.5">
                                    <span class="font-bold text-gray-900 text-xs font-mono whitespace-nowrap">
                            ...
                            <td class="py-3 px-3 sm:px-4 align-top border-r border-gray-100">
                                <div class="flex flex-col gap-1">
                                    <div class="flex items-center gap-1.5 sm:gap-2">
                                        <span class="w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-black flex-shrink-0
                                            {{ $record->user_role === 'admin' ? 'bg-blue-100 text-blue-800' : 'bg-emerald-100 text-emerald-800' }}">
                                            {{ strtoupper(substr($record->user_name ?? 'S', 0, 1)) }}
                                        </span>
                                        <span class="font-bold text-gray-900 text-xs leading-tight truncate max-w-[120px]">
                                            {{ $record->user_name ?? 'System' }}
                                        </span>
                                    </div>
                                    <span class="text-[10px] text-gray-500 italic pl-6 sm:pl-8">
                                        Role: <span class="capitalize font-semibold text-gray-700">{{ $record->user_role ?? 'system' }}</span>
                                    </span>
                                </div>
                            </td>
                                ...
                                @if($record->candidate_name)
                                    <div class="flex flex-col gap-1">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="font-bold text-gray-800 text-xs truncate max-w-[100px] sm:max-w-[130px]">{{ $record->candidate_name }}</span>
                                        </div>
                            ...
                            <td class="py-3 px-3 sm:px-4 align-top border-r border-gray-100">
                                <div class="flex flex-col gap-1.5">
                                    <span class="font-bold text-xs text-gray-800 whitespace-nowrap">
                            ...
                            <td class="py-3 px-3 sm:px-4 align-top border-r border-gray-100">
                                <div class="flex flex-col gap-1 text-xs">
                                    <span class="font-mono text-gray-700 font-bold whitespace-nowrap">IP: {{ $record->ip_address ?? '127.0.0.1' }}</span>
                                    <span class="text-[10px] text-gray-400 truncate max-w-[120px] sm:max-w-[140px]" title="{{ $record->user_agent }}">
                            ...
                            <td class="py-3 px-3 sm:px-4 align-top border-r border-gray-100">
                                <div class="flex flex-col gap-1">
                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border {{ $riskBadgeClass }} whitespace-nowrap">
                                        <span>{{ $riskIcon }}</span>
                                        <span>{{ $riskLabel }}</span>
                                    </span>
                                </div>
                            ...
                            <td class="py-3 px-3 sm:px-4 align-top text-center">
                                <div class="flex flex-col items-center gap-1 max-w-[90px] mx-auto">
                                    {{-- Add Note Modal Trigger --}}
                                    <button type="button"
                                            @click="openNoteModal({{ $record->id }}, '{{ route('admin.settings.audit_record.review', $record->id) }}')"
                                            class="w-full px-2 py-1 text-[10px] font-semibold bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg border border-gray-300 flex items-center justify-center gap-1 transition-colors">
                                        📝 Note
                                    </button>
                                    ...
                                    <form action="{{ route('admin.settings.audit_record.flag', $record->id) }}" method="POST" class="w-full">
                                        @csrf
                                        <button type="submit" class="w-full px-2 py-1 text-[10px] font-semibold {{ $record->is_suspicious ? 'bg-amber-100 text-amber-800 border-amber-300' : 'bg-gray-50 text-gray-600 border-gray-200' }} hover:bg-amber-200 rounded-lg border flex items-center justify-center gap-1 transition-colors">
